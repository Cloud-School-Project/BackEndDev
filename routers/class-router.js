...
router.get("/class", (req, res) => {
  findClasses()
    ...
    .catch((err) => {
      res
        .status(500)
        .json({ message: "Failed to get Classes", message: err.message });
    });
});
...
router.delete("/class", validClass, async (req, res) => {
  ...
  deleteClass(newId)
    ...
    .catch((err) => {
      res
        .status(500)
        .json({ message: "Failed to delete class, might not exist", err });
    });
});
...
//Middle for classes
function classUpdateCheck(req, res, next) {
  if (Boolean(req.body.morning || req.body.afternoon || req.body.evening)) {
    next();
  } else {
    res
      .status(500)
      .json({ message: "Need to have a Volunteer id to pass to time slot" });
  }
}
