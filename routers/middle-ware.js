  ...
  findVolunteerById(req.body.username)
    .then((response) => {
      if (response.length < 1) {
        findVolunteerByEmail(req.body.email).then((result) => {
          if (result.length < 1) {
            next();
          } else {
...
function checkStudent(req, res, next) {
  findStudentById(req.body.username)
    .then((response) => {
      if (response.length < 1) {
        findStudentByEmail(req.body.email).then((result) => {
          if (result.length < 1) {
            next();
          } else {
            res.status(401).json({ message: "user already exists" });
...
function restricted(req, res, next) {
  ...
  if (!token) {
    //if no token exists then error is thrown and middleware stops operation
  ...
  getLoggedOutList({ test: token }) // we pass the token in as an object(test:token)to our logged out list finder
    .then((resd) => {
      //the response will be ALL of the logged out tokens
      ...
      if (result.length === 1) {
        //if result holds a value then it did exisit on our logged out list
        return res.status(404).json("Token is on the logged out list");
      }
      jwt.verify(token, jwtSecret, (err, decoded) => {
        //else we verify that this token is still valid
        if (err) {
